"""Training / evaluation entry point for TRIDENT.
...
Total objective:
    L = L1(y_hat, y)
        + a_red * L_redundancy + a_uni * L_unique
        + a_syn * L_synergy    + a_rec * L_reconstruction
        + w_rank * L_rank
...
from losses import (redundancy_infonce, unique_orthogonality, synergy_exclusivity,
...
def compute_loss(cfg, out, y, l1):
    ...
    l_red = redundancy_infonce(out["C"], tau=cfg.infonce_tau)
    l_uni = unique_orthogonality(out["C"], out["U"])
    l_syn = synergy_exclusivity(out["S"], out["syn_preds"], grl_lambda=cfg.grl_lambda)
    l_rec = reconstruction_loss(out["proj_pool"], out["recon"])
    l_dis = (cfg.a_red * l_red + cfg.a_uni * l_uni
             + cfg.a_syn * l_syn + cfg.a_rec * l_rec)
    ...
    parts = {"total": total.item(), "main": main.item(), "dis": l_dis.item(),
             "rank": l_rank.item(), "aux": l_aux.item()}
    return total, parts
