...
class HyperbolicIntensityHead(nn.Module):
    ...
    def forward(self, z: torch.Tensor, reversal: torch.Tensor = None):
        """Returns (y_hat, radius, point_on_manifold).

        ``reversal`` (rho in [0, 1], shape (B,)) is the conflict-driven
        polarity-reversal gate. It flips the *direction* (polarity) while the
        intensity (geodesic radius) is conflict-invariant:

            polarity' = (1 - 2 * rho) * polarity

        rho = 0 keeps the surface polarity, rho = 1 reverses it (sarcasm), and
        rho = 0.5 neutralises it. When ``reversal`` is None the head behaves as a
        standard hyperbolic intensity head.
        """
        c = self.curvature
        v = self.proj(z)
        ...
        if reversal is not None:
            polarity = (1.0 - 2.0 * reversal) * polarity   # conflict-induced flip
